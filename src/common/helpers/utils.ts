    ...
    registerDecorator({
      ...
      validator: {
        validate(value: string) {
          ...
          if (phoneNumber) {
            return (
              phoneNumber.isValid() && isValidPhoneNumber(value) && phoneNumber.number == value
            );
          }
          return false;
        }
...
export function TransferPhoneNumberToOurStandard(mobileNumber: string): string {
  ...
  export function extractUpdatableFields (dto,fieldsToUpdate?:string[]){
     const fields = fieldsToUpdate ?? Object.keys(dto);
      const updateObj = fields.reduce((acc, key) => {
